#!/usr/bin/env bash
# =============================================================================
# PiCam H.264 Benchmarking Script (FB0 Edition)
# =============================================================================
...
#
# IMPORTANT CHANGES:
# - Display path is now ALWAYS the Linux framebuffer (fbdev) for on-screen output.
# - The script auto-detects the framebuffer pixel format (rgb565le/xrgb8888) and
#   scales to the current fb geometry for reliable rendering.
# - SDL/windowed output has been removed to ensure deterministic "camera → H.264
#   → decode → /dev/fb0" behavior. CLI flags are preserved; --fb0/--framebuffer
#   is accepted for compatibility but fbdev is always used.
#
# Features:
# - Auto-detection of CSI and USB cameras
# - Interactive menu and CLI interface
# - Real-time performance monitoring (overlay optional)
# - Dependency checks (optional auto-install preserved)
...
DEFAULT_METHOD="h264_sdl_preview"    # Back-compat name; now renders to fb0 internally
...
DEFAULT_FBDEV="/dev/fb0"            # Framebuffer device to display on
...
Options:
  ...
  -d, --duration <seconds>    Recording duration in seconds (default: ${DEFAULT_DURATION}, 0 = infinite)
      --fb0, --framebuffer    (Compatibility flag) Output is always fbdev; this flag is accepted but not required
      --fbdev <path>          Framebuffer device path (default: ${DEFAULT_FBDEV})
...
Examples:
  ${SCRIPT_NAME}
  ${SCRIPT_NAME} --method h264_sdl_preview --resolution 1920x1080 --fps 25 --bitrate 6000000
  ${SCRIPT_NAME} -s /dev/video0 -e hardware --fbdev /dev/fb0
USAGE
}
...
find_font_path() {
  ...
  if command -v find &>/dev/null; then
    local found_font
    found_font=$(find /usr/share/fonts -name "*.ttf" -print -quit 2>/dev/null || true)
...
REQUIRED_COMMANDS_BASE=(
  libcamera-vid    # Camera capture (older Raspberry Pi OS) OR rpicam-vid
...
OPTIONAL_COMMANDS_BASE=(
  ...
  fbset            # Framebuffer geometry detection
)
...
check_camera_command() {
  if command -v libcamera-vid >/dev/null 2>&1; then return 0; fi
  if command -v rpicam-vid   >/dev/null 2>&1; then return 0; fi
  return 1
}
...
is_usb_capture_node() {
  ...
  if command -v v4l2-ctl >/dev/null 2>&1; then
    ...
    local caps_line
    caps_line=$(grep -E '^[[:space:]]*Capabilities[[:space:]]*:.*0x' <<<"$info" | head -1 || true)
...
get_camera_type() {
  case "$SOURCE" in
    csi)
      echo "csi"; return;;
    /dev/video*)
      if [[ -c "$SOURCE" ]]; then echo "usb"; else echo "none"; fi
      return;;
    auto) ;;
...
get_encoding_method() {
  ...
  case "$ENCODE" in
    ...
    hardware) if detect_hardware_encoder; then echo "hardware"; else echo "software"; fi ;;
    auto)     if detect_hardware_encoder; then echo "hardware"; else echo "software"; fi ;;
...
apt_update_and_install() {
  ...
  if [[ ${#_packages_ref[@]} -eq 0 ]]; then return 0; fi
  if ! command -v apt-get >/dev/null 2>&1; then
    echo "${SCRIPT_NAME}: apt-get is required to install packages automatically." >&2
    return 1
  fi
  if ! maybe_sudo apt-get update; then return 1; fi
  if ! maybe_sudo apt-get install -y "${_packages_ref[@]}"; then return 1; fi
}
...
check_dependencies() {
  ...
  # Extra: ensure ffmpeg has fbdev output
  if command -v ffmpeg >/dev/null 2>&1; then
    if ! ffmpeg -hide_banner -devices 2>/dev/null | grep -qE 'D[[:space:]]+fbdev'; then
      echo "[WARN] ffmpeg 'fbdev' output device not listed. Your ffmpeg build may not support framebuffer output."
    fi
  fi
...
ensure_dependencies() {
  ...
  if check_dependencies "$require_whiptail"; then return 0; fi
  if [[ "$auto_install" -eq 0 ]]; then return 1; fi
  echo "Missing dependencies detected."
  if ! attempt_dependency_install "$require_whiptail"; then return 1; fi
  return 0
}
...
validate_corner() {
  case "$1" in
    ...
    *) die "Invalid overlay corner '${1}'. Use: top-left, top-right, bottom-left, bottom-right.";;
  esac
}
...
parse_arguments() {
  local parsed
  parsed=$(getopt -o m:r:f:b:c:s:e:d:vh --long method:,resolution:,fps:,bitrate:,corner:,source:,encode:,duration:,verbose,quiet,log-file:,help,menu,no-menu,no-overlay,check-deps,install-deps,debug-cameras,test-usb,list-cameras,fb0,framebuffer,fbdev: -- "$@") || {
    usage; exit 1; }
  eval set -- "$parsed"
  ...
  while true; do
    case "$1" in
      ...
      --fb0|--framebuffer) USE_FRAMEBUFFER=1; shift ;; # accepted for compatibility
...
show_whiptail_wizard() {
  ...
  menu_choice=$(whiptail --title "PiCam Benchmark" --menu "Select capture method" 20 78 10 \
    "h264_sdl_preview" "Camera -> H264 -> fbdev (fullscreen)" \
    3>&1 1>&2 2>&3) || exit 1
  METHOD="$menu_choice"
  ...
  local encode_options=("auto" "Auto-detect (hardware preferred)" "software" "Software encoding (CPU)")
  if detect_hardware_encoder; then
    encode_options+=("hardware" "Hardware encoding (Pi H.264)")
  fi
...
fb_geometry() {
  # Outputs "WIDTHxHEIGHT BPP", fallback to RESOLUTION and 32 bpp
  local w="$WIDTH" h="$HEIGHT" bpp="32"
  if command -v fbset >/dev/null 2>&1; then
    local g
    g=$(fbset -s 2>/dev/null | tr -s ' ')
...
fb_pix_fmt_from_bpp() {
  ...
  case "$bpp" in
    16) echo "rgb565le" ;;
    24) echo "bgr24"    ;;  # safer for many fb drivers if 24bpp (rare)
    30) echo "x2rgb10le" ;; # uncommon
    32) echo "xrgb8888" ;;
    *)  echo "xrgb8888" ;;
  esac
}
...
ffmpeg_has_drawtext() {
  ffmpeg -hide_banner -filters 2>/dev/null | grep -q '^ *T.*drawtext'
}

ffmpeg_has_fbdev_out() {
  ffmpeg -hide_banner -devices 2>/dev/null | grep -qE 'D[[:space:]]+fbdev'
}

# Simple process helpers
any_pid_alive() {
  local pid
  for pid in "$@"; do
    [[ -z "$pid" ]] && continue
...
run_camera_debug_report() {
  ...
  echo "Preferred camera command: $camera_cmd"; echo
  ...
  if command -v libcamera-vid >/dev/null 2>&1; then echo "libcamera-vid -> $(command -v libcamera-vid)"; else echo "libcamera-vid not found in PATH"; fi
  if command -v rpicam-vid   >/dev/null 2>&1; then echo "rpicam-vid   -> $(command -v rpicam-vid)"; else echo "rpicam-vid not found in PATH"; fi
  echo
  ...
  if command -v find >/dev/null 2>&1; then
    ...
    if [[ -n "$find_output" ]]; then echo "$find_output"; else echo "No camera binaries found in standard locations."; fi
  else
...
run_usb_camera_test() {
  ...
  echo
  parse_resolution "$RESOLUTION"
  local width="$WIDTH" height="$HEIGHT" usb_device="" input_format=""
  if ls /dev/video* >/dev/null 2>&1; then
    if command -v v4l2-ctl >/dev/null 2>&1; then
      for device in /dev/video*; do
        [[ -c "$device" ]] || continue
        ...
        if echo "$formats" | grep -q "H264"; then usb_device="$device"; input_format="h264"; echo "Found H264 capable device: $device"; break
        elif echo "$formats" | grep -q "MJPG"; then usb_device="$device"; input_format="mjpeg"; echo "Found MJPEG capable device: $device"; break
        elif echo "$formats" | grep -q "YUYV"; then usb_device="$device"; input_format="yuyv422"; echo "Found YUYV capable device: $device"; break
        fi
      done
    else
      for device in /dev/video*; do [[ -c "$device" ]] || continue; usb_device="$device"; input_format="mjpeg"; break; done
    fi
  fi
  if [[ -z "$usb_device" ]]; then echo "No suitable USB camera detected."; return 1; fi
  echo "Selected device: $usb_device with format: $input_format"
  if ! command -v ffmpeg >/dev/null 2>&1; then echo "ffmpeg is required for the USB camera test." >&2; return 1; fi
  local capture_cmd
  ...
  local timeout_cmd=(); command -v timeout >/dev/null 2>&1 && timeout_cmd=(timeout 6s)
  ...
  echo; echo "USB camera test completed successfully."
}

# =============================================================================
# DISPLAY (FBDEV) PIPELINE
# =============================================================================

# Compute drawtext position and return overlay-x/y
compute_overlay_position() {
  local corner="$1"
  case "$corner" in
    top-left)      echo "10 10" ;;
    top-right)     echo "w-tw-10 10" ;;
    bottom-left)   echo "10 h-th-10" ;;
    bottom-right)  echo "w-tw-10 h-th-10" ;;
    *)             echo "10 10" ;;
  esac
}

# Start a stats monitor that writes into a text file for drawtext
monitor_metrics() {
  local stats_file="$1"   # text for drawtext
  local ffmpeg_log="$2"   # display ffmpeg log to parse
  local width="$3" height="$4" bitrate_target="$5" fps_target="$6"
  shift 6
  local pids=("$@")
  ...
  while any_pid_alive "${pids[@]}"; do
    ...
    local usage cpu_usage mem_usage
...
# =============================================================================
# CAPTURE PIPELINES (CSI, USB) -> H.264 FIFO
# =============================================================================

# CSI camera using libcamera/rpicam → H.264 (inline) → FIFO
run_csi_h264_capture() {
  parse_resolution "$RESOLUTION"
  local width="$WIDTH" height="$HEIGHT" fps="$FPS" bitrate="$BITRATE"
  local duration_ms=0
  if [[ "$DURATION" -gt 0 ]]; then duration_ms=$((DURATION * 1000)); fi
  local cam_cmd; cam_cmd="$(get_camera_command)"
  log_info "Starting CSI camera: ${cam_cmd} ${width}x${height}@${fps}fps, bitrate=${bitrate}, timeout=${duration_ms}ms"
  stdbuf -oL "$cam_cmd" --inline --codec h264 --timeout "$duration_ms" \
    --width "$width" --height "$height" --framerate "$fps" \
    --bitrate "$bitrate" -o - > "$1" &
  echo $!
}

# Decide USB input format (H264/MJPG/YUYV) via v4l2-ctl if present
pick_usb_input_format() {
  local dev="$1"
  local fmt=""
  if command -v v4l2-ctl >/dev/null 2>&1; then
    local fmts
    fmts=$(v4l2-ctl --device="$dev" --list-formats-ext 2>/dev/null || true)
    if grep -q "H264" <<<"$fmts"; then fmt="h264"
    elif grep -q "MJPG" <<<"$fmts"; then fmt="mjpeg"
    elif grep -q "YUYV" <<<"$fmts"; then fmt="yuyv422"
    fi
  fi
  echo "$fmt"
}

# USB camera via ffmpeg → (copy or encode) → H.264 FIFO
run_usb_h264_capture() {
  parse_resolution "$RESOLUTION"
  local width="$WIDTH" height="$HEIGHT"
  local usb_device="$SOURCE"
  if [[ ! "$usb_device" =~ ^/dev/video[0-9]+$ ]]; then
    local detection; detection=$(detect_cameras); eval "$detection"
    if [[ "$usb_available" -ne 1 ]]; then die "No USB camera found"; fi
    usb_device="$usb_device"
  fi

  local infmt; infmt="$(pick_usb_input_format "$usb_device")"
  local encoding_method; encoding_method="$(get_encoding_method)"
  local duration_args=(); [[ "$DURATION" -gt 0 ]] && duration_args=(-t "$DURATION")

  log_info "Starting USB camera: ${usb_device} ${width}x${height}@${FPS} input=${infmt:-auto} encode=${encoding_method}"

  local input_args=(-f v4l2 -video_size "${width}x${height}" -framerate "${FPS}" -i "${usb_device}")
  if [[ -n "$infmt" ]]; then input_args=(-f v4l2 -input_format "$infmt" -video_size "${width}x${height}" -framerate "${FPS}" -i "${usb_device}"); fi
...
# =============================================================================
# FBDEV DISPLAY LEG (H.264 FIFO → decode → fbdev)
# =============================================================================

start_fb_display() {
  local fifo_in="$1"   # H.264 input FIFO
  local stats_file="$2"
  local ffmpeg_log="$3"
  local overlay_corner="$4"
  local fbdev="$5"
  ...
  log_info "Framebuffer ${fbdev}: geometry=${geo}, bpp=${bpp}, pix_fmt=${fmt}"

  # Overlay
  local drawtext=""
  if [[ "$NO_OVERLAY" -eq 0 ]] && ffmpeg_has_drawtext; then
    local font_path=""; find_font_path font_path
    local ox oy; read -r ox oy <<<"$(compute_overlay_position "$overlay_corner")"
    if [[ -n "$font_path" ]]; then
      drawtext="drawtext=fontfile=$(escape_path_for_drawtext "$font_path"):textfile=$(escape_path_for_drawtext "$stats_file"):reload=1:x=${ox}:y=${oy}:fontcolor=white:fontsize=28:box=1:boxcolor=0x000000AA:boxborderw=8:line_spacing=6"
    else
      drawtext="drawtext=textfile=$(escape_path_for_drawtext "$stats_file"):reload=1:x=${ox}:y=${oy}:fontcolor=white:fontsize=28:box=1:boxcolor=0x000000AA:boxborderw=8:line_spacing=6"
    fi
  elif [[ "$NO_OVERLAY" -eq 0 ]]; then
    log_info "ffmpeg drawtext filter not found; disabling overlay."
    NO_OVERLAY=1
  fi

  local vf_chain="scale=${geo}"
  if [[ -n "$drawtext" ]]; then
    vf_chain="${drawtext},${vf_chain}"
  fi

  # Ensure ffmpeg has fbdev output
  if ! ffmpeg_has_fbdev_out; then
    log_error "Your ffmpeg build may not support 'fbdev' output. Attempting anyway."
  fi

  # Start display leg first (consumer), so FIFO writers don't block
  stdbuf -oL -eL ffmpeg -hide_banner -loglevel info -stats \
    ...
    -f h264 -i "$fifo_in" \
    -vf "${vf_chain}" -pix_fmt "${fmt}" -an -f fbdev "${fbdev}" \
...
# =============================================================================
# TOP-LEVEL METHOD DISPATCH (ALWAYS FBDEV)
# =============================================================================
...
start_capture() {
  ...
  case "$METHOD" in
    # Back-compat: the method name still says "sdl", but we render to fbdev.
    h264_sdl_preview|h264_fb_preview|h264_fb0_preview)
      case "$camera_type" in
        csi)
          echo "Using CSI camera module..."
          run_csi_h264_pipeline_fbdev
          ;;
        usb)
          echo "Using USB camera..."
          run_usb_h264_pipeline_fbdev
          ;;
        none)
...
# CSI → H.264 (inline) → FIFO → decode → FBDEV
run_csi_h264_pipeline_fbdev() {
  parse_resolution "$RESOLUTION"
  overlay_position "$OVERLAY_CORNER"

  local tmpdir video_fifo stats_file ffmpeg_log
  tmpdir="$(mktemp -d)"
  ...
  local ffmpeg_pid="" camera_pid="" monitor_pid=""

  stop_process() { local pid="$1"; [[ -n "$pid" ]] || return; kill "$pid" 2>/dev/null || true; wait "$pid" 2>/dev/null || true; }
  cleanup_pipeline() {
    stop_process "$monitor_pid"; stop_process "$camera_pid"; stop_process "$ffmpeg_pid"
    rm -rf "$tmpdir" "$stats_file" "$ffmpeg_log"
  }
  trap cleanup_pipeline EXIT INT TERM

  ffmpeg_pid="$(start_fb_display "$video_fifo" "$stats_file" "$ffmpeg_log" "$OVERLAY_CORNER" "$FBDEV")"

  camera_pid="$(run_csi_h264_capture "$video_fifo")"

  if [[ "$NO_OVERLAY" -eq 0 ]]; then
    monitor_metrics "$stats_file" "$ffmpeg_log" "$WIDTH" "$HEIGHT" "$BITRATE" "$FPS" "$camera_pid" "$ffmpeg_pid" & monitor_pid=$!
  fi
  ...
  log_info "CSI camera pipeline completed."
  trap - EXIT INT TERM
  cleanup_pipeline
}

# USB → (copy/encode) H.264 → FIFO → decode → FBDEV
run_usb_h264_pipeline_fbdev() {
  parse_resolution "$RESOLUTION"
  overlay_position "$OVERLAY_CORNER"
  ...
  camera_pid="$(run_usb_h264_capture "$video_fifo")"
  ...
  log_info "USB camera pipeline completed."
  trap - EXIT INT TERM
  cleanup_pipeline
}
...
main() {
  ...
  USE_FRAMEBUFFER=1                  # Forced: we always render to fbdev
  FBDEV="$DEFAULT_FBDEV"            # Allow override via --fbdev
  SKIP_MENU=0
  FORCE_MENU=0
  NO_OVERLAY=0
  CHECK_DEPS_ONLY=0
  ...
  if [[ -n "$LOG_FILE" ]]; then log_info "Logging to file: $LOG_FILE"; fi
  log_info "${SCRIPT_NAME} starting: ${RESOLUTION}@${FPS}fps, bitrate=${BITRATE}, source=${SOURCE}, encode=${ENCODE}, duration=${DURATION}s, fbdev=${FBDEV}"
  log_debug "Method: ${METHOD}, Overlay corner: ${OVERLAY_CORNER}"

  # Warn users that SDL path is removed (for clarity)
  if [[ "${METHOD}" == "h264_sdl_preview" ]]; then
    log_info "Method 'h264_sdl_preview' selected; note: display is forced to framebuffer (fbdev) in this version."
  fi
  ...
  if (( DEBUG_CAMERAS_ONLY )); then run_camera_debug_report; exit 0; fi
  if (( USB_TEST_ONLY )); then
    if ! ensure_dependencies 0 1; then die "Dependencies remain missing; cannot run USB camera test."; fi
    if run_usb_camera_test; then exit 0; else exit 1; fi
  fi
  if (( LIST_CAMERAS_ONLY )); then list_available_cameras; exit 0; fi
  if (( CHECK_DEPS_ONLY )); then
    if ensure_dependencies "$require_whiptail" 0; then exit 0; else exit 1; fi
  fi
  if (( INSTALL_DEPS_ONLY )); then
    if ensure_dependencies "$require_whiptail" 1; then exit 0; else die "Dependency installation failed."; fi
  fi
  ...
  # Extra: sanity for fbdev device
  [[ -e "$FBDEV" ]] || die "Framebuffer device '$FBDEV' not found."
  [[ -w "$FBDEV" ]] || log_info "Framebuffer '$FBDEV' not writable by current user; try sudo."

  if [[ "$show_menu" -eq 1 ]]; then show_whiptail_wizard; fi
